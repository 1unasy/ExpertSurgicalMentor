...
```bash
./src/scripts/imitation_learning/record_object_dataset.sh more pick_and_place_v2 glasses 40
./src/scripts/imitation_learning/record_object_dataset.sh more pick_and_place_v2 xray 40
```

추가 에피소드를 더 수집할 때도 `more`를 사용한다.

```bash
./src/scripts/imitation_learning/record_object_dataset.sh more pick_and_place_v2 syringe 10
```
...
./src/scripts/imitation_learning/run_act_object.sh glasses
./src/scripts/imitation_learning/run_act_object.sh pill
./src/scripts/imitation_learning/run_act_object.sh xray
```
...
./src/scripts/imitation_learning/run_act_object.sh syringe --action-steps 100
```

follower 포트가 udev 링크로 설정되어 있으면 환경 변수로 변경할 수 있다.

```bash
ROBOT_PORT=/dev/omx_follower ./src/scripts/imitation_learning/run_act_object.sh syringe
```
...
MODEL_PREFIX=act_v2_full100k ./src/scripts/imitation_learning/run_act_object.sh syringe \
...
현재 교육용 감기 시나리오는 허용 목록에서 `감기/cold → syringe`로 매핑한다. 실행 순서는
질병 입력 검증 → Main Tray syringe 검출 → 손 안전 감시가 적용된 ACT 실행 → Assist Tray 결과
검증 → 안전하게 재시도 가능한 경우 1회 재시도이다.

물품 위치 검증에는 YOLO11s 사전학습 가중치로 학습한 2-class Object YOLO
`outputs/train/object_yolo_v1/weights/best.pt`를 사용한다. 클래스는 `pill`, `syringe`이며,
감기 시나리오에서는 `syringe` 클래스만 검증한다. ACT는 고정된
`act_v2_full100k_syringe/checkpoints/050000/pretrained_model`을 선택한다. `pill`을 직접 실행할
때도 `act_v2_full100k_pill/checkpoints/050000/pretrained_model`을 사용한다. 손 안전 감지는
YOLO11 n/s/m 비교에서 가장 좋은 결과를 낸 YOLO11s 모델
`outputs/train/hand_yolo_s_sweep_stable_v1/weights/best.pt`가 기본값이다.

실행기의 안전 분기는 다음과 같다.

- 시작 전 Object YOLO가 목표 물체를 Main Tray에서 확정하지 못하면 종료 코드 3으로 중단하며,
  ACT 명령은 전송하지 않는다.
- ACT 1회가 끝나면 로봇은 먼저 초기 자세로 돌아오고, Object YOLO가 Assist Tray를 확인한다.
- Assist Tray에서 확인되면 성공으로 종료한다.
- Assist Tray에는 없지만 Main Tray에서 다시 확정되면 초기 자세에서 ACT를 재시도한다.
- 어느 트레이에도 있다고 확정할 수 없는 `unknown` 상태에서는 물체를 떨어뜨렸거나 로봇이
  잡고 있을 가능성이 있으므로 자동 재시도하지 않고 종료 코드 4로 중단한다.

처음 한 번, front 카메라와 두 트레이를 최종 위치에 고정한 뒤 ROI를 지정한다. 대각선
트레이를 지원하도록 Main Tray와 Assist Tray의 외곽점을 각각 시계 방향으로 클릭한다. 기본적으로
3~8개 점을 지원하므로 사각형뿐 아니라 5각형·6각형 ROI도 사용할 수 있다. 마우스 오른쪽 클릭은
마지막 점 취소, `R`은 현재 선택 초기화, `Enter`는 3개 이상 점 저장, `Esc`는 취소이다.

```bash
source ~/venv/il/bin/activate
cd ~/ExpertSurgicalMentor
python ./src/scripts/tray/calibrate_tray_rois.py --camera 4
```
...
환자명은 실제 개인정보 대신 가상 식별자를 전달한다.

```bash
PATIENT_NAME="환자A"
DISEASE="감기"

echo "환자명: ${PATIENT_NAME}, 질환: ${DISEASE}"
...
환자명과 질환을 화면에서 입력하려면 Flask 로컬 웹 UI를 실행한다. 최초 한 번 Flask를
설치한 뒤 실행하면 브라우저에서 `http://127.0.0.1:5050`이 열린다.
...
웹 UI 왼쪽의 `의료 장비 확인 화면`에는 사전검사와 사후검사에서 생성한 최신 판정 프레임이
표시된다. 노란 선은 Main Tray, 하늘색 선은 Assist Tray이고, 목표 물체의 검출 상자에는
클래스·confidence·`MAIN/ASSIST/OUTSIDE` 판정이 표시된다. 로그의 `votes`는 여러 프레임 중
각 위치로 판정된 횟수이며, `max_consecutive`는 같은 위치에서 연속으로 확인된 최대 프레임
수다. 기본값은 총 10프레임 중 같은 트레이에서 연속 5프레임 확인이다. Assist Tray에서
연속 5프레임이 확인되면 성공으로 판정하고 추가 ACT 명령 없이 파이프라인을 자동 종료한다.
ACT가 동작하는 동안 별도 Object YOLO를 계속 실행하면 카메라와
GPU 경합으로 제어 주기가 느려질 수 있으므로 마지막 사전검사 프레임을 유지하고, 동작 완료
후 사후검사 결과로 갱신한다.

UI의 `긴급 정지 (소프트웨어)` 버튼은 실행 중인 전체 프로세스 그룹에 `SIGINT`를 보내 안전한
종료 처리를 요청한다. 이는 통신 장애나 프로세스 정지 상황에서 동작을 보장하는 물리 비상정지
장치를 대체하지 않는다.

오른쪽의 `의료 장비 전달 수행 화면`에는 ACT가 이미 읽고 있는 front 카메라 관측을 약 5Hz로 전달한다.
Flask가 `/dev/video4`를 별도로 열지 않고 LeRobot 제어 루프의 프레임을 재사용하므로 카메라
중복 점유가 없다. 이 화면에는 Object YOLO를 추가 실행하지 않으며 사람이 작업 진행 상태를
확인하기 위한 용도다.

checkpoint와 재시도 횟수를 지정할 수도 있다.

```bash
./src/scripts/pipeline/run_cold_scenario.sh cold \
  --checkpoint 050000 \
  --action-steps 100 \
  --episode-time 30 \
  --max-retries 1
