...
# Reuses src/scripts/hand_yolo/train_hand_yolo.sh for each run, then prints and
# saves a comparison table read from each run's results.csv (best-fitness epoch).
#
...
#   OPTIMIZER, LR0                           forwarded to train_hand_yolo.sh
#   OUTPUT_ROOT project root under which runs are saved (default: outputs/train)
#
# Example:
#   VARIANTS="n s m" EPOCHS=100 BATCH=8 DEVICE=0 \
...
for V in $VARIANTS; do
  ...
  MODEL="$MODEL_PT" RUN_NAME="$RUN_NAME" OUTPUT_ROOT="$OUTPUT_ROOT" \
    "$PROJECT_ROOT/src/scripts/hand_yolo/train_hand_yolo.sh"
done
