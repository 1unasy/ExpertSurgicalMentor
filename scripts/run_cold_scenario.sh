...
VERIFIED_RUNNER="${VERIFIED_RUNNER:-$PROJECT_ROOT/scripts/run_verified_act_object.sh}"
...
HAND_MODEL="${SAFETY_YOLO_MODEL:-$PROJECT_ROOT/outputs/train/hand_yolo_v1/weights/best.pt}"
...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/run_cold_scenario.sh [감기|cold] [options]
...
Pipeline:
  ...
  2. Verify pill is in Main Tray with object YOLO.
  3. Run the pill ACT policy with YOLO hand interruption enabled.
  4. Verify pill moved to Assist Tray; retry only when safely possible.

Options:
  --checkpoint NAME     ACT checkpoint (default: last)
  --action-steps N      ACT actions before replanning (default: 100)
  --episode-time SEC    Maximum ACT execution time (default: 30)
  --max-retries N       Additional attempts if pill remains in Main (default: 1)
...
Examples:
  ./scripts/run_cold_scenario.sh --dry-run
  ./scripts/run_cold_scenario.sh 감기 --checkpoint 050000
  ./scripts/run_cold_scenario.sh cold --max-retries 2
EOF
}
...
[[ -f "$ROI_CONFIG" ]] || {
  ...
  echo "Run: python scripts/calibrate_tray_rois.py --camera 4" >&2
  errors=$((errors + 1))
}
